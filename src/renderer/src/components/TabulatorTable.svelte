<script>
  ...
  const columns = [
    ...
    { title: 'Status', field: 'status', editor: 'list',width:170, editorParams: { values: ['queued','running','done','error'] } },
    { title: 'Created', field: 'createdAt', sorter: 'number',
      formatter: cell => new Date(cell.getValue()).toLocaleString() },
    { title: '', width: 44, hozAlign: 'center',
